	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) (err error) {
			...
			st.AlwaysRun(idl.Substep_execute_finalize_data_migration_scripts, func(streams step.OutStreams) error {
				...
				return commanders.ApplyDataMigrationScripts(nonInteractive, conf.Target.GPHome, conf.Target.CoordinatorPort(),
...
If you choose to postpone after the upgrade run "vacuumdb --all --analyze-in-stages".`)
					fmt.Println()
				...
				return conf.Target.RunGreenplumCmd(streams, "vacuumdb", "--all", "--analyze-only")
			})
...
"vacuumdb --all --analyze-in-stages"`,
				...
				idl.Step_finalize,
				conf.Target.GPHome, conf.Target.CoordinatorPort(), filepath.Join(response.GetLogArchiveDirectory(), "data-migration-scripts"), idl.Step_finalize,
			))
		},
	}
