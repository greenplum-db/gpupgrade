...
func TestGenerateDataMigrationScripts(t *testing.T) {
	...
	t.Run("errors when failing to create output directory", func(t *testing.T) {
		...
		err := commanders.GenerateDataMigrationScripts(false, "", 0, "", outputDirFS, "", fstest.MapFS{})
	...
	t.Run("returns without error when archiving scripts is skipped", func(t *testing.T) {
		...
		err := commanders.GenerateDataMigrationScripts(false, "", 0, "", outputDirFS, "", outputDirFS)
		if err != nil {
	...
	t.Run("does not error when plpythonu is present", func(t *testing.T) {
		...
		err = commanders.GenerateDataMigrationScripts(true, "/usr/local/gpdb5", 0, "", seedDirFS, outputDir, fstest.MapFS{})
		if err != nil {
			t.Errorf("unexpected error: %#v", err)
		}
	})
